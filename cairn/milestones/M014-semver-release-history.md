...
- 2026-09-04: step-7 approval: PR #21 approved for merge (user at the gate); fix-now D-006 committed on the branch before the marker.
...
- 2026-09-04 re-review findings (three lenses): [O] four — (1) D-005 "bodies unchanged" overstated after the compare-link retarget: fix now, D-006 annotating D-005 (user at the gate); (2) v1.0.0 at a README-only commit — re-raises prior finding 9, noted; (3) v1.1.3 folds pre-existing state — re-raises prior finding 8, noted; (4) `M014-bodies/` merges into main — by design, deleted at archive, noted. [O] confirmed prior fixes 1–4 landed. [S] history: no conflicts. [S] prior-review: no findings (README paragraph does not reintroduce M12's Hub-unfriendly-lines issue); PR inline-comment probe empty.
- 2026-09-04 conversation: PR #21 — no reviews, no comments, no unresolved threads.
