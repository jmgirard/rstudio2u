...
- 2026-09-03 review fan-out: [O] diff-bug lens 9 findings; [S] blame-history lens no conflicts, one wording-parity observation; [S] prior-review lens "no regressions of prior review points" (M08/M09 archives read; `gh api pulls/comments` empty). Findings, ranked, with proposed disposition (final at the gate):
  - F1 (fix now): both stubs' `image inspect` ignores its argument and `config --images` is a clean fixed line, so the CR strip / both-ends trim and the per-image loop have no discriminating coverage — a launcher inspecting a hardcoded name passes.
  - F2 (fix now): the warning-absent assertion is keyed on pull success, so on `pull-failure` / `offline-config-unsupported` a launcher printing the warning and then the hard error would be green.
  - F3 (fix now): "Could not download the latest image" is a substring of both branch messages, so `pull-failure`'s text assertion cannot tell the branches apart; only exit code and the forbidden `compose up` discriminate.
  - F4 (reject): README/CHANGELOG say the launcher reuses the downloaded image unconditionally, but a Compose without `config --images` still gets the hard error — the launchers target current Docker Desktop, and the README statement is the AC4 text.
  - F5 (reject): bash strips CR but not spaces, batch trims spaces but not CR — `for /f` never delivers a CR and POSIX Compose never pads names, so neither gap has a real input.
  - F6 (fix now): a successful but empty `config --images` (the `found=0` guard) is untested on both platforms.
  - F7 (fix now): the POSIX stub logs calls after the `info` handler, so `docker info` is never logged, unlike the PowerShell stub.
  - F8 (reject): `:images_present` leaves `IMAGE`/`IMAGES_FOUND` in the caller's environment — harmless, nothing else uses them.
  - F9 (follow-up candidate): the offline warning prints before the up-to-180s health wait, so it has scrolled off by the time the "running" banner shows.
  - History-lens wording observation (reject): both platforms print "Could not download the latest image, so the update was skipped." — verified in launcher_common.sh:50 and start_windows.bat:86.
- 2026-09-03 gate triage (user): fix now F1, F2, F3, F6, F7; follow-up candidate F9; rejected F4, F5, F8 and the wording observation. Fix-now landed in both harnesses only: stubs list two images (first with a CR on POSIX, space-padded on Windows), `image inspect` succeeds only for a listed name, `STUB_IMAGE_ABSENT=<name>` and `STUB_CONFIG_EMPTY=1` added, scenarios `offline-second-image-absent` and `offline-config-empty` added, hard-error scenarios assert "Check your internet connection" and forbid the warning, `offline-fallback` forbids the hard error, POSIX stub logs `info`. POSIX: 54/54 ok; four planted defects (hardcoded name, CR unstripped, first-image-only, warning before hard error) each turned the expected scenarios red. Windows verified by CI on the pushed commit.
- 2026-09-03: step-7 approval: PR #17 approved for merge (chip option "fix F1-F3, F6, F7 first, then merge"); CI green on 893815b (runs 33814461753, 33814461773).
