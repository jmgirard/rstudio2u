...
if [ -f "/usr/lib/rstudio-server/bin/pandoc/pandoc" ]; then
    ...
    # (e.g. .../tools/aarch64/pandoc or .../tools/x86_64/pandoc); take the
    # first match, if any.
    for candidate in /usr/lib/rstudio-server/bin/quarto/bin/tools/*/pandoc; do
        if [ -f "$candidate" ]; then
            BUNDLED_PANDOC="$candidate"
            break
        fi
    done
fi
